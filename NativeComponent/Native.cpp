...
using namespace NativeComponent::Notifications;


#pragma region ShellChrome

void ShellChrome::TurnScreenOn(bool state)
{
...
#pragma region NotificationManager

NotificationManager::NotificationManager(int resWidth, int resHeight)
{
	wstringstream sstm;
...
Snapshot^ NotificationManager::GetNotificationsSnapshot()
{
	...
	if (shellRes == S_OK)
	{
		Snapshot^ mSnap = ref new Snapshot();
		...
		for (unsigned int i = 0; i < pSnapshot->detailedTextCount; i++)
		{
			...
			dtInfo->IsBoldText = pSnapshot->detailedTexts[i].isBoldText == TRUE;
...
Array<uint8>^ NotificationManager::GetUIXMAResource(String^ name)
{
